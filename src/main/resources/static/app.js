...
const MOCK_MODE = true; // false로 바꾸면 실제 API 호출
...
let mockStats = [
    [3, '채식주의자', 42], [7, '82년생 김지영', 38], [12, '아몬드', 31],
    [5, '데미안', 27],    [9, '어린 왕자', 25],     [14, '소년이 온다', 19],
];
...
function refreshCurrentPage() {
    if (currentPage === 'list' && document.getElementById('list-member-id').value) loadRentals();
...
function formatDate(str) { return str ? str.replace(/-/g, '.') : '—'; }
...
async function loadRentals() {
    ...
    tbody.innerHTML = `<tr><td colspan="9" style="text-align:center;padding:30px;"><span class="spinner-sm" style="width:20px;height:20px;border-width:3px;"></span></td></tr>`;
    try {
        allRentals = await apiGet(`/member/${id}`);
        renderRentals(allRentals);
        updateStatCards(allRentals);
    } catch (e) {
        tbody.innerHTML = `<tr><td colspan="9"><div class="empty-state"><i class="bi bi-exclamation-circle"></i><p>${e.message}</p></div></td></tr>`;
...
function renderRentals(list) {
    const tbody = document.getElementById('rental-tbody');
    if (!list.length) {
        tbody.innerHTML = `<tr><td colspan="9"><div class="empty-state"><i class="bi bi-inbox"></i><p>대출 내역이 없습니다</p></div></td></tr>`;
    ...
    tbody.innerHTML = list.map(r => {
        ...
        const daysText = r.status === 'RETURNED' ? '—'
            : days < 0   ? `<span class="overdue-days">+${Math.abs(days)}일 연체</span>`
                : days === 0 ? `<span style="color:var(--amber);font-weight:600;font-size:0.78rem;">오늘 마감</span>`
                    : `<span class="td-mono">${days}일 남음</span>`;
        const actionBtns = r.status !== 'RETURNED'
            ? `<button class="btn-sm-action bsa-return me-1" onclick="quickReturn(${r.rentalId})"><i class="bi bi-box-arrow-up"></i> 반납</button>
         ${r.renewCount < 1 ? `<button class="btn-sm-action bsa-renew" onclick="quickRenew(${r.rentalId})"><i class="bi bi-arrow-repeat"></i> 연장</button>` : ''}`
...
async function quickRenew(rentalId) {
    if (!confirm(`대출 #${rentalId} 도서를 연장하시겠습니까?`)) return;
...
function openDetail(rentalId) {
  ...
  `;
    document.getElementById('detail-footer').innerHTML = r.status !== 'RETURNED'
       ...
       ${r.renewCount < 1 ? `<button class="btn-ghost" style="color:var(--amber);border-color:var(--amber);" onclick="quickRenew(${r.rentalId});closeDetailPanel()"><i class="bi bi-arrow-repeat"></i> 연장</button>` : ''}
       <button class="btn-teal" onclick="quickReturn(${r.rentalId});closeDetailPanel()"><i class="bi bi-box-arrow-up"></i> 반납 처리</button>`
...
async function submitRent() {
    ...
    try {
        ...
        showAlert('rent-alert', `도서 #${bookId} 대출 완료! (반납 기한: 14일)`, 'success');
...
async function loadQuickReturn() {
    ...
    if (!id) return;
    tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;padding:20px;"><span class="spinner-sm" style="width:18px;height:18px;border-width:2px;"></span></td></tr>`;
    try {
        const list = await apiGet(`/member/${id}`);
        if (!list.length) { tbody.innerHTML = `<tr><td colspan="5"><div class="empty-state" style="padding:24px;"><i class="bi bi-inbox"></i><p>대출 중인 도서가 없습니다</p></div></td></tr>`; return; }
        tbody.innerHTML = list.map(r => `
      <tr>
        <td class="td-mono" style="color:var(--muted);">#${r.rentalId}</td>
        ...
        <td class="td-mono">${formatDate(r.dueDate)}</td>
        <td>${statusBadge(r)}</td>
        <td><button class="btn-sm-action bsa-return" onclick="quickReturnAndRefresh(${r.rentalId},'${id}')"><i class="bi bi-box-arrow-up"></i> 반납</button></td>
      </tr>`).join('');
    } catch (e) {
        tbody.innerHTML = `<tr><td colspan="5"><div class="empty-state"><i class="bi bi-exclamation-circle"></i><p>${e.message}</p></div></td></tr>`;
    }
}
...
async function loadStats() {
    ...
    tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;padding:30px;"><span class="spinner-sm" style="width:20px;height:20px;border-width:3px;"></span></td></tr>`;
    try {
        const list = await apiGet('/stats');
        if (!list.length) { tbody.innerHTML = `<tr><td colspan="5"><div class="empty-state"><i class="bi bi-inbox"></i><p>데이터가 없습니다</p></div></td></tr>`; return; }
        ...
        <td><span style="font-family:'IBM Plex Mono',monospace;font-size:0.9rem;font-weight:600;">${count}</span><span style="font-size:0.72rem;color:var(--muted);margin-left:4px;">회</span></td>
        <td>
    ...
    } catch (e) { toast('통계 로드 실패: ' + e.message, 'error'); }
}
...
(function init() {
    ...
    document.getElementById('topbar-date').textContent =
        `${d.getFullYear()}.${String(d.getMonth()+1).padStart(2,'0')}.${String(d.getDate()).padStart(2,'0')} ${dayNames[d.getDay()]}요일`;
})();